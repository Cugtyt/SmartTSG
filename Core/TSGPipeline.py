        ...
        for i, input_key in enumerate(step.inputs):
            if '.' in input_key:
                ...
                if step_name not in self.store:
                    logging.error(f"Step {step_name} not found in store: {self.store}")
                    logging.error(f"Please check the order of steps in TSG file")
                    return False
                if output_key not in self.store[step_name]:
                    logging.error(f"Output key {output_key} not found in store: {self.store}")
                    logging.error(f"Please check the order of steps in TSG file")
                    return False
